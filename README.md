...
`Node-Vue-Moba`是一个面向广大王者荣耀玩家的全栈项目，基于Vue+Element实现。后台管理端主要包括分类管理，新闻管理，英雄管理，视频管理，运营管理等。采用了通用的CRUD接口，便于管理员对数据进行修改，符合开闭原则。前端展示界面所调用的接口与后台管理端分离，实现高内聚，低耦合，方便进行修改及扩展字段。
